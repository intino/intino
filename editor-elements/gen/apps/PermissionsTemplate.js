...
import PermissionsTemplatePage from "../pages/PermissionsTemplatePage";
...
var launchApplication = function () {
	...
	function renderApplication() {
		...
		const content = requireGoogleApi ? <GoogleApiLoader onLoad={openPushService()}><PermissionsTemplatePage/></GoogleApiLoader> : <PermissionsTemplatePage/>;
		const permissionsTemplate = document.getElementById("PermissionsTemplate");
		if (permissionsTemplate) ReactDOM.render(<DisplayRouter id="__router__" owner={()=>""} context={()=>""}>{content}</DisplayRouter>, permissionsTemplate);
