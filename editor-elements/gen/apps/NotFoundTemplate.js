...
import NotFoundTemplatePage from "../pages/NotFoundTemplatePage";
...
var launchApplication = function () {
	...
	function renderApplication() {
		...
		const content = requireGoogleApi ? <GoogleApiLoader onLoad={openPushService()}><NotFoundTemplatePage/></GoogleApiLoader> : <NotFoundTemplatePage/>;
		const notFoundTemplate = document.getElementById("NotFoundTemplate");
		if (notFoundTemplate) ReactDOM.render(<DisplayRouter id="__router__" owner={()=>""} context={()=>""}>{content}</DisplayRouter>, notFoundTemplate);
...
function loadAppUrl() {
	...
	url = url.replace(new RegExp("\\/not-found", 'g'), "");
