...
var launchApplication = function () {
	...
	function renderApplication() {
		...
		const content = requireGoogleApi ? <GoogleApiLoader onLoad={openPushService()}><LoginTemplatePage/></GoogleApiLoader> : <LoginTemplatePage/>;
		const loginTemplate = document.getElementById("LoginTemplate");
		if (loginTemplate) ReactDOM.render(<DisplayRouter id="__router__" owner={()=>""} context={()=>""}>{content}</DisplayRouter>, loginTemplate);
...
function loadAppUrl() {
	...
	url = url.replace(new RegExp("\\/login", 'g'), "");
