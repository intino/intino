...
import io.quassar.editor.box.ui.pages.LoginPage;
...
public class LoginResource extends io.intino.alexandria.ui.server.resources.Resource {
	private final EditorBox box;

	public LoginResource(EditorBox box, io.intino.alexandria.ui.server.AlexandriaUiManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return LoginResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((LoginPage)page).box = box;
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null && box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher) {
				((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchLogin(client.soul());
				return false;
			}
			...
			if (box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher)
			    ((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchLogin(soul);
