...
import io.quassar.editor.box.ui.pages.ForgePage;
...
public class ForgeResource extends io.intino.alexandria.ui.server.resources.Resource {
	private final EditorBox box;

	public ForgeResource(EditorBox box, io.intino.alexandria.ui.server.AlexandriaUiManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return ForgeResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((ForgePage)page).box = box;
		...
		((ForgePage)page).model = parameterValue("model");
		((ForgePage)page).release = parameterValue("release");
		((ForgePage)page).view = parameterValue("view");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null && box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher) {
				((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchForge(client.soul(), ((ForgePage)page).model, ((ForgePage)page).release, ((ForgePage)page).view);
				return false;
			}
			...
			if (box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher)
			    ((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchForge(soul, ((ForgePage)page).model, ((ForgePage)page).release, ((ForgePage)page).view);
