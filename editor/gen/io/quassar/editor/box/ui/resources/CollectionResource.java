...
import io.quassar.editor.box.ui.pages.CollectionPage;
...
public class CollectionResource extends io.intino.alexandria.ui.server.resources.Resource {
	private final EditorBox box;

	public CollectionResource(EditorBox box, io.intino.alexandria.ui.server.AlexandriaUiManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		UiPage page = new CollectionPage();
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return CollectionResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((CollectionPage)page).box = box;
		...
		((CollectionPage)page).collection = parameterValue("collection");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null && box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher) {
				((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchCollection(client.soul(), ((CollectionPage)page).collection);
				return false;
			}
			...
			if (box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher)
			    ((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchCollection(soul, ((CollectionPage)page).collection);
