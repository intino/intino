...
import io.quassar.editor.box.ui.pages.LanguageActionPage;
...
public class LanguageActionResource extends io.intino.alexandria.ui.server.resources.Resource {
	private final EditorBox box;

	public LanguageActionResource(EditorBox box, io.intino.alexandria.ui.server.AlexandriaUiManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		UiPage page = new LanguageActionPage();
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return LanguageActionResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((LanguageActionPage)page).box = box;
		...
		((LanguageActionPage)page).language = parameterValue("language");
		((LanguageActionPage)page).action = parameterValue("action");
		((LanguageActionPage)page).version = parameterValue("version");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null && box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher) {
				((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchLanguageAction(client.soul(), ((LanguageActionPage)page).language, ((LanguageActionPage)page).action, ((LanguageActionPage)page).version);
				return false;
			}
			...
			if (box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher)
			    ((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchLanguageAction(soul, ((LanguageActionPage)page).language, ((LanguageActionPage)page).action, ((LanguageActionPage)page).version);
