...
public class ModelResource extends io.intino.alexandria.ui.server.resources.Resource {
	private final EditorBox box;

	public ModelResource(EditorBox box, io.intino.alexandria.ui.server.AlexandriaUiManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return ModelResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((ModelPage)page).box = box;
		...
		((ModelPage)page).language = parameterValue("language");
		((ModelPage)page).model = parameterValue("model");
		((ModelPage)page).release = parameterValue("release");
		((ModelPage)page).tab = parameterValue("tab");
		((ModelPage)page).view = parameterValue("view");
		((ModelPage)page).file = parameterValue("file");
		((ModelPage)page).pos = parameterValue("pos");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null && box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher) {
				((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchModel(client.soul(), ((ModelPage)page).language, ((ModelPage)page).model, ((ModelPage)page).release, ((ModelPage)page).tab, ((ModelPage)page).view, ((ModelPage)page).file, ((ModelPage)page).pos);
				return false;
			}
			...
			if (box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher)
			    ((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchModel(soul, ((ModelPage)page).language, ((ModelPage)page).model, ((ModelPage)page).release, ((ModelPage)page).tab, ((ModelPage)page).view, ((ModelPage)page).file, ((ModelPage)page).pos);
