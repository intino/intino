...
import io.quassar.editor.box.ui.pages.HomePage;
...
public class HomeResource extends io.intino.alexandria.ui.server.resources.Resource {
	private final EditorBox box;

	public HomeResource(EditorBox box, io.intino.alexandria.ui.server.AlexandriaUiManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		((HomePage)page).dialog = parameterValue("dialog");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null && box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher) {
				((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchHome(client.soul(), ((HomePage)page).dialog);
				return false;
			}
			...
			if (box.routeManager().routeDispatcher() instanceof io.quassar.editor.box.ui.displays.RouteDispatcher)
			    ((io.quassar.editor.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchHome(soul, ((HomePage)page).dialog);
