...
public abstract class AbstractModelLanguageToolbar<B extends Box> extends io.intino.alexandria.ui.displays.components.Template<TemplateNotifier, java.lang.Void, B> {
	...
	public AbstractModelLanguageToolbar(B box) {
		super(box);
		id("modelLanguageToolbar");
	}

	@Override
	public void init() {
		super.init();
		if (aboutText == null) aboutText = register(new AboutText(box()).<AboutText>id("a1587946260").owner(AbstractModelLanguageToolbar.this));
		if (aboutLink == null) aboutLink = register(new AboutLink(box()).<AboutLink>id("a1587711457").owner(AbstractModelLanguageToolbar.this));
		if (helpText == null) helpText = register(new HelpText(box()).<HelpText>id("a690138132").owner(AbstractModelLanguageToolbar.this));
		if (helpLink == null) helpLink = register(new HelpLink(box()).<HelpLink>id("a689903329").owner(AbstractModelLanguageToolbar.this));
		if (examplesText == null) examplesText = register(new ExamplesText(box()).<ExamplesText>id("a_1450196452").owner(AbstractModelLanguageToolbar.this));
		if (examplesLink == null) examplesLink = register(new ExamplesLink(box()).<ExamplesLink>id("a_1450431255").owner(AbstractModelLanguageToolbar.this));
		if (languageLogo == null) languageLogo = register(new LanguageLogo(box()).<LanguageLogo>id("a824285737").owner(AbstractModelLanguageToolbar.this));
	}
