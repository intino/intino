...
public class GetDownloadModelFileResource implements Resource {
	...
	public GetDownloadModelFileResource(EditorBox box, AlexandriaHttpManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.quassar.editor.box.actions.GetDownloadModelFileAction()).execute());
	}

	private io.quassar.editor.box.actions.GetDownloadModelFileAction fill(io.quassar.editor.box.actions.GetDownloadModelFileAction action) throws AlexandriaException {
		action.box = this.box;
		action.context = context();
		try {
			action.commit = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromPath("commit"), String.class);
			action.file = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromPath("file"), String.class);
		} catch (Throwable e) {
