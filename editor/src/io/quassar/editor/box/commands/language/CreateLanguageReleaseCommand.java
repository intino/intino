...
public class CreateLanguageReleaseCommand extends Command<CommandResult> {
	...
	private BuildResult build(Model metamodel) {
		...
		try {
			...
			box.languageManager().saveLsp(language, version, LanguageHelper.lspOf(destination));
			box.languageManager().saveLspManifest(language, version, LanguageHelper.lspManifestOf(destination));
	...
	private void copyExample(Language language, LanguageRelease release, String example) {
		...
		copy.language(GavCoordinates.from(language, release));
	...
	private Model copyTemplate(Language language, LanguageRelease release, LanguageRelease previousRelease) {
		...
		clone.language(GavCoordinates.from(language, release));
		clone.name(language.name());
