...
public class ModelCommands extends Commands {
	...
	public Model create(String name, String description, GavCoordinates language, String mainContent, String owner, String username) {
		CreateModelCommand command = setup(new CreateModelCommand(box), username);
		command.name = name;
		command.description = description;
		command.language = language;
		command.owner = owner;
		command.mainContent = mainContent;
		return command.execute();
	}

	public Model create(String name, String description, GavCoordinates language, String owner, String username) {
		return create(name, description, language, null, owner, username);
	}
	...
	public void saveDescription(Model model, String description, String username) {
		SaveModelDescriptionCommand command = setup(new SaveModelDescriptionCommand(box), username);
		command.model = model;
		command.description = description;
		command.execute();
	}

	public void saveSimpleName(Model model, String title, String username) {
		SaveModelSimpleNameCommand command = setup(new SaveModelSimpleNameCommand(box), username);
		command.model = model;
		command.title = title;
		command.execute();
	}

	public void saveQualifiedName(Model model, String project, String module, String username) {
		SaveModelQualifiedNameCommand command = setup(new SaveModelQualifiedNameCommand(box), username);
		command.model = model;
	...
	public Model createModule(String name, String project, GavCoordinates language, String username) {
		CreateModuleCommand command = setup(new CreateModuleCommand(box), username);
		command.name = name;
		command.project = project;
		command.language = language;
		return command.execute();
