...
public class ModelManager {
	...
	public List<GavCoordinates> languages(String project, String owner) {
		Set<Subject> result = new HashSet<>(subjectStore.query().isType(SubjectHelper.ModelType).where("owner").equals(owner).where("project").equals(project).collect());
		result.addAll(subjectStore.query().isType(SubjectHelper.ModelType).where("collaborator").equals(owner).where("project").equals(project).collect());
		return result.stream().map(this::get).map(Model::language).distinct().toList();
	}

	public List<Model> models(String project, String module, String owner) {
		Set<Subject> result = new HashSet<>(subjectStore.query().isType(SubjectHelper.ModelType).where("owner").equals(owner).where("project").equals(project).where("module").equals(module).collect());
		result.addAll(subjectStore.query().isType(SubjectHelper.ModelType).where("collaborator").equals(owner).where("project").equals(project).where("module").equals(module).collect());
		return result.stream().map(this::get).toList();
	}

	public List<String> projects(String owner) {
		Set<Subject> result = new HashSet<>(subjectStore.query().isType(SubjectHelper.ModelType).where("owner").equals(owner).collect());
		result.addAll(subjectStore.query().isType(SubjectHelper.ModelType).where("collaborator").equals(owner).collect());
	...
	public Model model(String project, String module, GavCoordinates language, String owner) {
		if (project == null || module == null) return null;
		Set<Subject> result = new HashSet<>(subjectStore.query().isType(SubjectHelper.ModelType).where("owner").equals(owner).where("project").equals(project).where("module").equals(module).where("dsl-collection").equals(language.groupId()).where("dsl-name").equals(language.artifactId()).where("dsl-version").equals(language.version()).collect());
		result.addAll(subjectStore.query().isType(SubjectHelper.ModelType).where("collaborator").equals(owner).where("project").equals(project).where("module").equals(module).where("dsl-collection").equals(language.groupId()).where("dsl-name").equals(language.artifactId()).where("dsl-version").equals(language.version()).collect());
		return !result.isEmpty() ? get(new ArrayList<>(result).getFirst()) : null;
	}
	...
	public Model create(String id, String name, String description, GavCoordinates language, Model.Usage usage, String owner) {
