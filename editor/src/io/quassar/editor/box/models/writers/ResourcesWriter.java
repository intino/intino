...
public class ResourcesWriter implements FileWriter {
	...
	public File createFolder(String name, File parent) {
		...
		destiny.mkdirs();
		return new File(destiny.getName(), relativePath(destiny, workspace), true, new ArrayList<>());
	}
	...
	public File rename(File file, String newName) {
		try {
			...
			return new File(destiny.getName(), relativePath(destiny, workspace), file.isDirectory(), new ArrayList<>());
		} catch (IOException e) {
			Logger.error(e);
			return null;
		}
	...
	public File copy(String filename, File sourceFile) {
		try {
			...
			return new File(destiny.getName(), relativePath(destiny, workspace), sourceFile.isDirectory(), new ArrayList<>());
		} catch (IOException e) {
			Logger.error(e);
			return null;
		}
