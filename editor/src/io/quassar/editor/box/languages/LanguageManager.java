...
public class LanguageManager {
	...
	public void saveLsp(Language language, String release, File lsp) {
		copy(lsp, archetype.languages().releaseLspJar(language.key(), release));
		LanguageRelease languageRelease = language.release(release);
		ArtifactoryHelper.cleanLspCache(language, languageRelease, archetype.languages());
	}

	public void saveLspManifest(Language language, String release, File lsp) {
		copy(lsp, archetype.languages().releaseLspManifest(language.key(), release));
		LanguageRelease languageRelease = language.release(release);
		ArtifactoryHelper.cleanLspCache(language, languageRelease, archetype.languages());
	}

	public File loadLsp(GavCoordinates coordinates) {
		return loadLsp(coordinates.languageId(), coordinates.version());
	}

	public File loadLsp(Language language, LanguageRelease release) {
		return loadLsp(language.key(), release.version());
	}

	public File loadLspDigest(Language language, LanguageRelease release) {
		File digest = archetype.languages().releaseLspJarDigest(language.key(), release.version());
		if (!digest.exists()) ArtifactoryHelper.prepareLsp(language, release, archetype.languages());
		return digest.exists() ? digest : null;
	}

	public File loadLspManifest(Language language, LanguageRelease release) {
		File manifest = archetype.languages().releaseLspManifest(language.key(), release.version());
		if (!manifest.exists()) ArtifactoryHelper.prepareLsp(language, release, archetype.languages());
		return manifest.exists() ? manifest : null;
	}

	public File loadLspManifestDigest(Language language, LanguageRelease release) {
		File manifest = archetype.languages().releaseLspManifestDigest(language.key(), release.version());
		if (!manifest.exists()) ArtifactoryHelper.prepareLsp(language, release, archetype.languages());
		return manifest.exists() ? manifest : null;
	}
	...
	public Language getMetta() {
		return get(Language.key(Language.QuassarCollection, Language.Metta));
	}
	...
	private File loadLsp(String language, String release) {
		if (release == null) return null;
		File file = archetype.languages().releaseLspJar(language, release);
		if (!file.exists()) return null;
		return file;
	}
	...
	private void resize(File logo, int size, File destiny) throws IOException {
		if (destiny.exists()) destiny.delete();
		ImageResizer.resizeImage(logo, size, destiny);
	}
