...
public class LocalLanguageArtifactory implements LanguageArtifactory {
	...
	public String mainClass(GavCoordinates gav) {
		...
		return StringHelper.kebabCaseToCamelCase(groupId.replace("_", "-")).toLowerCase() + "." + Formatters.firstUpperCase(StringHelper.kebabCaseToCamelCase(gav.artifactId().replace("_", "-")));
	}
}
