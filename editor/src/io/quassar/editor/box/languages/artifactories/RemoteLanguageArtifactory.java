...
public class RemoteLanguageArtifactory implements LanguageArtifactory {
	...
	public String mainClass(GavCoordinates gav) {
		...
		if (model == null) {
			...
			return groupId + "." + Formatters.firstUpperCase(StringHelper.kebabCaseToCamelCase(gav.artifactId().replace("_","-")));
		}
	...
	public File download(GavCoordinates gav) throws IOException {
		...
		File file = archetype.languages().releaseDslJar(gav.languageId(), gav.version());
