...
public class ModelSettingsEditor extends AbstractModelSettingsEditor<EditorBox> {
	...
	private void initGeneralBlock() {
		...
		editName.onExecute(e -> openNameDialog());
		nameDialog.onSave(e -> {
			modelNameField.value(e);
			saveNameListener.accept(model);
		});
	...
	private void refreshGeneralBlock() {
		...
		editName.readonly(!PermissionsHelper.canEditName(model, box()));
		modelNameField.value(ModelHelper.label(model, language(), box()));
