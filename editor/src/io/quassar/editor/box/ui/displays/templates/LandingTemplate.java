...
public class LandingTemplate extends AbstractLandingTemplate<EditorBox> {
	...
	public void init() {
		...
		startBuildingLogin.onExecute(e -> gotoLogin(PathHelper.languageUrl(Language.Metta, session())));
	...
	private void startBuilding() {
		startBuilding(box().languageManager().getMetta());
	}
	...
	private void startBuilding(Language language) {
		...
		Model model = box().commands(ModelCommands.class).create(name, translate("(no description)"), GavCoordinates.from(language, release), username(), username());
