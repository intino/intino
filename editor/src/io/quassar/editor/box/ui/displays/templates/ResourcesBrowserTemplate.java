...
public class ResourcesBrowserTemplate extends AbstractResourcesBrowserTemplate<EditorBox> {
	...
	private void addFile() {
		changeListener.accept(box().commands(ModelCommands.class).createFile(model, nameOf(fileField.value()), new ByteArrayInputStream(new byte[0]), parentFile(), username()));
	}

	private void uploadFile() {
		try {
			changeListener.accept(box().commands(ModelCommands.class).createFile(model, nameOf(uploadedFile.name()), uploadedFile.stream(), parentFile(), username()));
		} catch (IOException e) {
			Logger.error(e);
		}
	}
	...
	private void addFolder() {
		changeListener.accept(box().commands(ModelCommands.class).createFolder(model, nameOf(fileField.value()), parentFile(), username()));
	}
	...
	private String nameOf(String name) {
		return file == null || !file.isDirectory() ? io.quassar.editor.box.models.File.ResourcesDirectory + File.separator + name : name;
	}

	private io.quassar.editor.box.models.File parentFile() {
