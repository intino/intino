...
public class LanguageExecutionTemplate extends AbstractLanguageExecutionTemplate<EditorBox> {
	...
	public void init() {
		...
		exampleCodeField.onChange(e -> copyExampleCodeUsage.text(e.value()));
	...
	private void refreshExamplesCodeUsage() {
		examplesCodeUsage.clear();
		ExamplesCodeUsage.forEach(l -> fillExampleCodeUsage(l, examplesCodeUsage.add()));
		updateExampleCodeUsage(ExamplesCodeUsage.getFirst());
	}
	...
	private String exampleCodeUsageContent(String name) {
		try {
			...
			if (stream == null) return "";
			return replaceVars(IOUtils.toString(stream, StandardCharsets.UTF_8));
		} catch (IOException e) {
			Logger.error(e);
			return "";
		}
	...
	private String replaceVars(String content) {
		content = content.replace("::dsl::", Formatters.firstUpperCase(Formatters.kebabCaseToCamelCase(language.name())));
		content = content.replace("::dslLower::", Formatters.kebabCaseToCamelCase(language.name()).toLowerCase());
		content = content.replace("::commitUrl::", PathHelper.commitUrl(session()));
		return content;
	}
	...
	private void fillExampleCodeUsage(String exampleCodeUsage, LinkTemplate display) {
		...
		display.onClick(e -> updateExampleCodeUsage(exampleCodeUsage));
		display.refresh();
	}
	...
	private void updateExampleCodeUsage(String name) {
		String value = exampleCodeUsageContent(name);
		copyExampleCodeUsage.text(value);
		exampleCodeField.value(value);
	}
