...
public class LanguageEditor extends AbstractLanguageEditor<EditorBox> {
	...
	private String metamodelTitle() {
		String result = metamodel.name().contains(".") ? metamodel.name().substring(metamodel.name().lastIndexOf(".")+1) : metamodel.name();
	...
	private void changeId() {
		...
		notifyUser(translate("Updating DSL properties..."), UserMessage.Type.Loading);
