...
public class HomeTemplate extends AbstractHomeTemplate<EditorBox> {
	...
	public void createModel(String languageId) {
		...
		Model model = box().commands(ModelCommands.class).create(name, "", new GavCoordinates(language.collection(), language.name(), language.lastRelease().version()), username(), username());
	...
	private void registerUserIfNeeded() {
		...
		box().commands(UserCommands.class).create(user.username(), user.fullName(), username());
	}
