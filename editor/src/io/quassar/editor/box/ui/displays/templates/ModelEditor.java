...
import static io.quassar.editor.box.util.DisplayHelper.*;
...
public class ModelEditor extends AbstractModelEditor<EditorBox> {
	...
	public void init() {
		...
		initSettings();
		collapsedTabSelector.onSelect(this::openTab);
	...
	private void initSettings() {
		...
		modelSettingsEditor.onSaveName(e -> refreshHeader());
	...
	private void refreshPanels() {
		expandLeftPanel.visible(!SessionHelper.isLeftPanelExpanded(session()));
		collapseLeftPanel.visible(SessionHelper.isLeftPanelExpanded(session()));
		collapsedLeftPanel.visible(!SessionHelper.isLeftPanelExpanded(session()));
		collapsedTabSelector.selection(selectedView == ModelView.Model ? "collapsedModel" : "collapsedResources");
		expandRightPanel.visible(!SessionHelper.isRightPanelExpanded(session()));
		expandRightPanelFull.visible(!SessionHelper.isRightPanelExpanded(session()));
		collapsedRightPanel.visible(!SessionHelper.isRightPanelExpanded(session()));
		modelEditionBlock.refreshLayout(SessionHelper.isLeftPanelExpanded(session()) ? SessionHelper.leftPanelSize(session()) : 0, -1.0, SessionHelper.isRightPanelExpanded(session()) ? SessionHelper.rightPanelSize(session()) : 0);
	}
	...
	private void initHeader() {
		...
		headerStamp.onOpenRelease(this::openRelease);
	...
	private void initEditionBlock() {
		expandLeftPanel.onExecute(e -> expandLeftPanel());
		collapseLeftPanel.onExecute(e -> collapseLeftPanel());
		expandRightPanel.onExecute(e -> expandRightPanel());
		expandRightPanelFull.onExecute(e -> expandRightPanel());
		languageExplorer.onCollapse(e -> collapseRightPanel());
		...
		modelEditionBlock.onLayoutChanged(this::updatePanelsSize);
	}
	...
	private void refreshModelBrowserBlock() {
		...
		if (display != null && display.view() == selectedView) {
			display.files(editorFiles());
			display.file(selectedFile, selectedPosition);
		}
		else intinoDslEditor.clear();
	}
	...
	private IntinoDslEditor createFileEditor() {
		...
		editor.onCommit(e -> commit());
		editor.onCheck(e -> check());
		return editor;
	}
...
//		if (display == null || !display.sameReleaseAndFile(release, selectedFile.uri())) display = createFileEditor();
	...
	private void expandLeftPanel() {
		boolean rightPanelExpanded = SessionHelper.isRightPanelExpanded(session());
		modelEditionBlock.refreshLayout(DefaultLeftPanelSize, rightPanelExpanded ? -1 : 78.0, SessionHelper.isRightPanelExpanded(session()) ? DefaultRightPanelSize : 0);
		SessionHelper.registerLeftPanelExpanded(session());
		expandLeftPanel.visible(false);
		collapseLeftPanel.visible(true);
		collapsedLeftPanel.visible(false);
		boolean modelBrowserVisible = modelBrowserBlock.isVisible();
		modelBrowserBlock.hide();
		resourcesBrowserBlock.hide();
		if (modelBrowserVisible) modelBrowserBlock.show();
		else resourcesBrowserBlock.show();
	}

	private void collapseLeftPanel() {
		boolean rightPanelExpanded = SessionHelper.isRightPanelExpanded(session());
		modelEditionBlock.refreshLayout(0.0, rightPanelExpanded ? -1 : 100.0, SessionHelper.isRightPanelExpanded(session()) ? DefaultRightPanelSize : 0.0);
		SessionHelper.registerLeftPanelCollapsed(session());
		expandLeftPanel.visible(true);
		collapseLeftPanel.visible(false);
		collapsedLeftPanel.visible(true);
	}

	private void expandRightPanel() {
		boolean leftPanelExpanded = SessionHelper.isLeftPanelExpanded(session());
		modelEditionBlock.refreshLayout(leftPanelExpanded ? DefaultLeftPanelSize : 0, leftPanelExpanded ? -1 : 68.0, DefaultRightPanelSize);
		SessionHelper.registerRightPanelExpanded(session());
		collapsedRightPanel.visible(false);
		languageExplorer.invalidateCache(true);
		languageExplorer.refresh();
	}

	private void collapseRightPanel() {
		boolean leftPanelExpanded = SessionHelper.isLeftPanelExpanded(session());
		modelEditionBlock.refreshLayout(leftPanelExpanded ? DefaultLeftPanelSize : 0, leftPanelExpanded ? -1 : 100.0, 0.0);
		SessionHelper.registerRightPanelCollapsed(session());
		collapsedRightPanel.visible(true);
		expandRightPanel.visible(true);
		expandRightPanel.refresh();
	}

	private void openRelease(String release) {
		intinoDslEditor.clear();
		notifier.dispatch(PathHelper.modelPath(model, release, selectedTab, selectedView, selectedFile));
	}

	private void openTab(SelectionEvent event) {
		if (event.selection().isEmpty()) return;
		ModelView view = event.selection().getFirst().equals("collapsedModel") ? ModelView.Model : ModelView.Resources;
		notifier.dispatch(PathHelper.modelPath(model, release, selectedTab, view));
	}

	private void updatePanelsSize(LayoutChangedEvent event) {
		Double leftPanelSize = event.layout().get(0);
		Double rightPanelSize = event.layout().get(2);
		SessionHelper.registerLeftPanelSize(session(), leftPanelSize > 0 ? leftPanelSize : DefaultLeftPanelSize);
