...
public class CollaboratorsDatasource extends PageDatasource<User> {
	...
	public CollaboratorsDatasource(EditorBox box, UISession session, List<String> blackList) {
		this.box = box;
	...
	private List<User> filterCondition(List<User> users, String condition) {
		...
		return users.stream().filter(m -> DatasourceHelper.matches(m.name(), conditions) || DatasourceHelper.matches(m.fullname(), conditions)).collect(toList());
	}
