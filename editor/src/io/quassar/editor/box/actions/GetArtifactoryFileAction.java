...
public class GetArtifactoryFileAction implements io.intino.alexandria.rest.RequestErrorHandler {
	...
	public io.intino.alexandria.Resource execute() throws Forbidden, NotFound {
		...
		boolean isLsp = coordinates.artifactId().equalsIgnoreCase(ArtifactoryHelper.LanguageServer);

		if (isDsl) return loadDsl(language, release, isManifest, isDigest);
		else if (isLsp) return loadLsp(language, release, isManifest, isDigest);
		return loadParser(language, release, coordinates.artifactId(), isManifest, isDigest);
	}
	...
	private Resource loadLsp(Language language, LanguageRelease release, boolean isManifest, boolean isDigest) throws NotFound {
		if (isManifest) return resourceOf(isDigest ? box.languageManager().loadLspManifestDigest(language, release) : box.languageManager().loadLspManifest(language, release));
		return resourceOf(isDigest ? box.languageManager().loadLspDigest(language, release) : box.languageManager().loadLsp(language, release));
	}
