...
public class ModelAccessorGenerator {
	...
	public boolean generate() {
		...
		if (pom != null) {
			...
			File build = new File(pom.getParentFile(), "out/build/" + conf.module().toLowerCase() + "-model-parser-java");
	...
	private void generateModelReaderClass() {
		try {
			FrameBuilder builder = new FrameBuilder().add("model")
			...
			String content = new Engine(new ModelTemplate()).render(builder.toFrame());
			File dir = new File(conf.srcDirectory(), conf.generationPackage().replace(".", "/"));
			dir.mkdirs();
			Files.writeString(new File(dir, firstUpperCase(conf.dsl().outDsl()) + "Model.java").toPath(), content);
		} catch (IOException e) {
