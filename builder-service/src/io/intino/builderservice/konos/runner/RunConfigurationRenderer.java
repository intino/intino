...
public class RunConfigurationRenderer {
	...
	public String build() {
		...
		writer.write(ARTIFACT_ID + NL + normalize(params.projectName())+ NL);
		writer.write(GROUP_ID + NL + normalize(params.projectGroup()) + NL);
		writePaths(writer);
		...
		writer.write(GENERATION_PACKAGE + NL + (normalize(params.generationPackage()).isEmpty() ? "model" : normalize(params.generationPackage())) + NL);
	...
	private String normalize(String name) {
		return StringFormatters.camelCase().format(name).toString().toLowerCase();
	}
