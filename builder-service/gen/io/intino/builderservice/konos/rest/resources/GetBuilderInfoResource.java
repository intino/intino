...
public class GetBuilderInfoResource implements Resource {
	...
	public GetBuilderInfoResource(BuilderServiceBox box, AlexandriaHttpManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.builderservice.konos.actions.GetBuilderInfoAction()).execute());
	}

	private io.intino.builderservice.konos.actions.GetBuilderInfoAction fill(io.intino.builderservice.konos.actions.GetBuilderInfoAction action) throws AlexandriaException {
		action.box = this.box;
		action.context = context();
		try {
			action.imageURL = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("imageURL"), String.class);
			action.registryToken = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromBody(), String.class);
		} catch (Throwable e) {
	...
	private void write(BuilderInfo object) {
