...
public class RemoveElementFix extends CodeAction {
	public RemoveElementFix(DiagnosticContextInfo info) {
		...
		List<Constraint.Property> constraints = findConstraints(info).stream().
				...
				map(constraint -> (Constraint.Property) constraint).collect(Collectors.toList());
		WorkspaceEdit edit = new WorkspaceEdit();
		edit.setChanges(Map.of(info.uri().toString(), List.of(positionateCursor())));
		setEdit(edit);
		setTitle("Remove element");
		setKind(QuickFix);
		String template = createTemplateFor(constraints);
		if (template != null) {
			Command command = new Command(getTitle(),
					"editor.action.insertSnippet",
					List.of(Map.<String, Object>of("snippet", template))
			);
		}
	}
	...
	private List<Constraint> findConstraints(DiagnosticContextInfo info) {
		...
		List<Constraint> constraints = info.language().constraints(mogram.types().getFirst());
		if (constraints == null || constraints.isEmpty()) return List.of();
		final List<Constraint> constraintsOf = new ArrayList<>(constraints);
