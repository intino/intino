...
class ContextFilters {
	...
	private static class AfterDefFitFilter implements Predicate<CompletionContext> {
		...
		public boolean test(CompletionContext context) {
			...
			if (parent != null && parent instanceof TaraGrammar.PropertyTypeContext) return true;
	...
	private static class AfterEqualsFilter implements Predicate<CompletionContext> {
		@Override
		public boolean test(CompletionContext context) {
			final ParserRuleContext realContext = TreeUtils.contextByType(context.ruleOnPosition(), TaraGrammar.ValueContext.class);
	...
	private static class InFacetFilter implements Predicate<CompletionContext> {
		...
		public boolean test(CompletionContext context) {
			return acceptableParent(context.tokenOnPosition(), context.ruleOnPosition()) &&
					...
					!(facet(context.ruleOnPosition()) && TreeUtils.getMogramContainerRuleOf(context.ruleOnPosition()) == null);
		}
	...
	private static class InParameters implements Predicate<CompletionContext> {
		...
		public boolean test(CompletionContext context) {
			return acceptableParent(context.mogramOnPosition(), context.ruleOnPosition()) &&
					...
					TreeUtils.getMogramContainerRuleOf(context.ruleOnPosition()) != null;
		}
...
//		return prevSibling(context) instanceof ParserRuleContext pc && pc.getToken(type, 0) != null;
	...
	private static class InIdenfierReference implements Predicate<CompletionContext> {
		@Override
		public boolean test(CompletionContext context) {
			ParserRuleContext rule = context.ruleOnPosition();
			while (rule != null) {
				if (rule instanceof TaraGrammar.IdentifierReferenceContext) return true;
				rule = rule.getParent();
			}
			return false;
		}
