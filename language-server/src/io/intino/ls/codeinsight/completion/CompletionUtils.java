...
public class CompletionUtils {
	...
	List<CompletionItem> collectBodyProperties(Mogram mogram) {
		...
		return buildCompletionForParameters(parameterConstraintsOf(mogram,true), mogram.parameters());
	}

	List<CompletionItem> collectSignatureParameters(Mogram mogram) {
		if (language == null || mogram == null) return null;
		String facet = isInFacet(context.ruleOnPosition(), mogram);
		List<Constraint.Property> propConstraints = facet != null ? collectParameterConstraints(mogram, facet) : parameterConstraintsOf(mogram,false);
		return buildCompletionForParameters(propConstraints, removeFakes(mogram));
	}
	...
	private String isInFacet(ParserRuleContext ruleContext, Mogram mogram) {
		TaraGrammar.FacetContext facetContext = TreeUtils.contextByType(context.ruleOnPosition(), TaraGrammar.FacetContext.class);
		if (facetContext == null) return null;
		String facetType = facetContext.metaidentifier().getText();
		return mogram.appliedFacets().stream()
	...
	private List<Constraint.Property> parameterConstraintsOf(Mogram mogram, boolean withFacet) {
		...
		for (Constraint constraint : constraints)
			...
			else if (constraint instanceof Constraint.Facet && withFacet && hasFacet(mogram, (Constraint.Facet) constraint))
	...
	private List<Constraint.Property> collectParameterConstraints(Mogram mogram, String facet) {
		List<Constraint> coreConstraints = language.constraints(mogram.types().getFirst());
	...
	private CompletionItem createCompletionItem(Constraint.Property c) {
		CompletionItem item = new CompletionItem(type(c) + " " + c.name() + facetContext(c));
	...
	private static String type(Constraint.Property c) {
		return c.type().equals(Primitive.REFERENCE) ? refrenceType(c) : c.type().getName();
	}
	...
	private static String refrenceType(Constraint.Property c) {
		...
		if (referenceRule != null) return referenceRule.allowedReferences().getFirst();
